...
#         self.data_len = len(fnmatch.filter(os.listdir(self.data_path + '/image'), '*.npy'))
    ...
    def __getitem__(self, i):
        ...
        image = cv2.resize(image, dsize=(384,384), interpolation=cv2.INTER_NEAREST)
        semantic = np.load(self.data_path + '/label/{:d}.npy'.format(index))
        semantic = cv2.resize(semantic, dsize=(384,384), 
                              interpolation=cv2.INTER_NEAREST)
        depth = np.load(self.data_path + '/depth/{:d}.npy'.format(index))
        depth = np.expand_dims(cv2.resize(depth,dsize=(384,384), 
                              interpolation=cv2.INTER_NEAREST),axis=-1)
        normal = np.load(self.data_path + '/normal/{:d}.npy'.format(index))
        normal = cv2.resize(normal,dsize=(384,384),interpolation=cv2.INTER_NEAREST)
        image = torch.from_numpy(np.moveaxis(image, -1, 0))
        semantic = torch.from_numpy(semantic)
        depth = torch.from_numpy(np.moveaxis(depth, -1, 0))
        depth1 = torch.from_numpy(np.moveaxis(np.load(self.data_path + '/depth/{:d}.npy'.format(index)), -1, 0))
        
        normal = torch.from_numpy(np.moveaxis(normal, -1, 0))
