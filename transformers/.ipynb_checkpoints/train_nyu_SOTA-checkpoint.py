...
def parse_args():
    ...
    parser.add_argument('--weighting', default='KD_MTL', type=str, 
...
if params.weighting=='KD_MTL':
    ...
    for i in range(task_num):
        single_model[i] = build_model(dataset='NYUv2', args=args, tasks=[model.tasks[i]],
                                      weighting=params.weighting, 
                                      random_distribution=params.random_distribution).cuda()
...
optimizer = optim.AdamW(opt, lr=1e-4, weight_decay=1e-5)
    ...
    for batch_index in range(train_batch):
        ...
        if params.weighting == 'KD_MTL':
            ...
            for i in range(task_num):
                ...
                feat_si0 = feat_si[0]
                feat_si0 = feat_si0 / (feat_si0.pow(2).sum(1) + 1e-6).sqrt().view(feat_si0.size(0), 
                                                                                1, feat_si0.size(2))
