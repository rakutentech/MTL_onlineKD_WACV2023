...
class transformer(torch.nn.Module):
    def __init__(self):
        ...
        self.conv4 = torch.nn.Conv2d(512, 512, 1, bias=False)
        self.conv5 = torch.nn.Conv2d(1024, 1024, 1, bias=False)
    ...
    def forward(self, inputs):
        results = []
        results.append(self.conv2(inputs[0]))
        results.append(self.conv3(inputs[1]))
        results.append(self.conv4(inputs[2]))
        results.append(self.conv5(inputs[3]))
        results.append(self.conv1(inputs[4]))
        return results
        ...
        if params.AFD==True:  
            ...
            for i in range(task_num):
                if i==0:
                    ...
                    sig = mtl.sigma3
                with torch.no_grad():
                     feat_ti = single_model[tasks[i]].get_features(train_data)
                dist_0=0.0
                feat_si = transformers[i](feat_s)
                
                for j in range(5):
                    feat_ti0 = feat_ti[j].detach()
                    feat_ti0 = feat_ti0 / (feat_ti0.pow(2).sum(1) + 1e-6).sqrt().view(feat_ti0.size(0), 
                                                                                      1, feat_ti0.size(2),
                                                                                      feat_ti0.size(3))            

                    feat_si0 = feat_si[j] / (feat_si[j].pow(2).sum(1) + 1e-6).sqrt().view(feat_si[j].size(0), 
                                                                                    1, feat_si[j].size(2),
                                                                                    feat_si[j].size(3))
                    dist_0 += (feat_si0 - sig[j]*feat_ti0).pow(2).sum(1).mean()/len(feat_ti)
